...
def plot_graph(df):
    ...
    if len(df.columns) < 2:
        st.error("Graph cannot be generated: Not enough columns.")
        return
    
    numeric_cols = df.select_dtypes(include=['number']).columns
    if len(numeric_cols) < 1:
        st.error("Graph cannot be generated: No numeric columns found.")
        return
...
5. Never give anything else but the SQL query and only give one query with no extra formatting.  
